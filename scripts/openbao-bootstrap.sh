...
kubectl apply -f "${project_dir}/infra/external-secrets/payroll/namespace.yaml" >/dev/null
kubectl apply -f "${project_dir}/infra/external-secrets/payroll/service-account.yaml" >/dev/null

OPENBAO_HOST_GATEWAY=$(docker network inspect "${k3d_network}" \
  --format '{{(index .IPAM.Config 0).Gateway}}')
export OPENBAO_HOST_GATEWAY
mkdir -p "${project_dir}/.generated"
envsubst <"${project_dir}/infra/external-secrets/payroll/openbao-endpoint-slice.yaml.tpl" \
  >"${project_dir}/.generated/openbao-endpoint-slice.yaml"
kubectl apply -f "${project_dir}/infra/external-secrets/payroll/openbao-service.yaml" >/dev/null
...
docker exec -i -e BAO_TOKEN="${BAO_TOKEN}" "${bao_container}" \
  bao policy write payroll-read - <"${project_dir}/infra/openbao/payroll-read.hcl" >/dev/null
if ! docker exec -e BAO_TOKEN="${BAO_TOKEN}" "${bao_container}" \
...
docker exec -e BAO_TOKEN="${BAO_TOKEN}" "${bao_container}" \
  bao write auth/kubernetes/role/payroll-dev \
    bound_service_account_names=payroll-secrets \
    bound_service_account_namespaces=payroll-dev \
    audience=vault alias_name_source=serviceaccount_name \
    policies=payroll-read ttl=1h >/dev/null
